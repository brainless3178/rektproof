...
pub struct AddGuardian<'info> {
    #[account(
        constraint = authority.key() == config.authority @ ProktorError::UnauthorizedAuthority,
    )]
...
pub fn add_guardian(ctx: Context<AddGuardian>, new_guardian: Pubkey) -> Result<()> {
    ...
    require!(
        ...
        ProktorError::GuardianSetFull
    );

    // Prevent duplicates
    require!(
        !config.guardians.contains(&new_guardian),
        ProktorError::DuplicateGuardian
    );
...
pub fn remove_guardian(ctx: Context<RemoveGuardian>, guardian: Pubkey) -> Result<()> {
    ...
    require!(
        ...
        ProktorError::GuardianBelowThreshold
    );
...
pub fn propose_authority_transfer(
    ...
    require!(
        ...
        ProktorError::PendingAuthorityTransferExists
    );
...
pub fn accept_authority_transfer(ctx: Context<AcceptAuthorityTransfer>) -> Result<()> {
    ...
    let pending = config
        .pending_authority
        .ok_or(ProktorError::NoPendingAuthorityTransfer)?;

    require!(
        ctx.accounts.new_authority.key() == pending,
        ProktorError::InvalidPendingAuthority
    );
...
pub fn cancel_authority_transfer(ctx: Context<CancelAuthorityTransfer>) -> Result<()> {
    ...
    require!(
        ...
        ProktorError::NoPendingAuthorityTransfer
    );
