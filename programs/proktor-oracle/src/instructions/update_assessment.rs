...
pub struct UpdateAssessment<'info> {
    ...
    #[account(
        ...
        constraint = analyst_account.active @ ProktorError::AnalystInactive,
        constraint = analyst_account.wallet == analyst_signer.key() @ ProktorError::UnauthorizedAnalyst,
    )]
    ...
    #[account(
        seeds = [CONFIG_SEED],
        bump = config.bump,
        constraint = !config.paused @ ProktorError::OraclePaused,
    )]
    ...
    #[account(
        ...
        constraint = risk_score.analyst == analyst_signer.key() @ ProktorError::UnauthorizedAnalyst,
        constraint = risk_score.status != AssessmentStatus::Superseded @ ProktorError::AssessmentSuperseded,
        constraint = risk_score.status != AssessmentStatus::Withdrawn @ ProktorError::AssessmentFinalized,
    )]
...
pub fn update_assessment(
    ...
    require!(
        ...
        ProktorError::TooManyFlags
    );
    require!(!flags.is_empty(), ProktorError::InvalidRiskScore);
    ...
    for input in &flags {
        require!(input.flag_id.len() <= 8, ProktorError::FlagIdTooLong);
        require!(
            input.description.len() <= MAX_FLAG_DESC_LEN,
            ProktorError::FlagDescriptionTooLong
        );
    ...
    risk_score.revision = risk_score
        .revision
        .checked_add(1)
        .ok_or(ProktorError::MathOverflow)?;
