...
mod tests {
    ...
    #[test]
    fn test_all_supported_vuln_types_produce_poc() {
        let vuln_ids = ["SOL-001", "SOL-002", "SOL-003", "SOL-005", "SOL-017", "SOL-019", "SOL-021", "SOL-033", "SOL-999"];
        for id in &vuln_ids {
            let finding = create_test_finding(id);
            let poc = AttackSimulator::generate_executable_poc(&finding);
            assert_eq!(poc.vulnerability_id, *id, "PoC should match vuln id");
            assert!(!poc.scenario_name.is_empty(), "{} should have scenario name", id);
            assert!(!poc.attack_steps.is_empty(), "{} should have attack steps", id);
            assert!(!poc.expected_outcome.is_empty(), "{} should have expected outcome", id);
        }
    }

    #[test]
    fn test_poc_includes_concrete_code_and_mitigations() {
        let finding = create_test_finding("SOL-002");
        let poc = AttackSimulator::generate_executable_poc(&finding);
        // Overflow PoC should have TS code with MAX_SAFE_INTEGER or u64::MAX
        assert!(poc.typescript_poc.is_some(), "should have TS PoC");
        let ts = poc.typescript_poc.unwrap();
        assert!(ts.len() > 50, "TS PoC should be non-trivial");
        assert!(!poc.mitigations.is_empty(), "should suggest mitigations");
    }

    #[test]
    fn test_generic_fallback_has_valid_structure() {
        let finding = create_test_finding("SOL-UNKNOWN-999");
        let poc = AttackSimulator::generate_executable_poc(&finding);
        assert_eq!(poc.vulnerability_id, "SOL-UNKNOWN-999");
        // Generic fallback should still have steps and prerequisites
        assert!(!poc.attack_steps.is_empty());
        assert!(!poc.prerequisites.is_empty());
    }

    #[test]
    fn test_reentrancy_poc_has_expected_steps() {
        let finding = create_test_finding("SOL-017");
        let poc = AttackSimulator::generate_executable_poc(&finding);
        assert_eq!(poc.vulnerability_id, "SOL-017");
        // Reentrancy PoC should have at least 3 steps (deploy, invoke, re-enter)
        assert!(
            poc.attack_steps.len() >= 3,
            "reentrancy PoC should have at least 3 attack steps, got {}",
            poc.attack_steps.len()
        );
    }
}
