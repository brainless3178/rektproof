...
mod tests {
    ...
    #[test]
    fn test_schema_based_input_generation() {
        let config = FuzzerConfig::default();
        let mut fuzzer = SecurityFuzzer::new(config);
        let schema = FuzzInputSchema {
            fields: vec![
                FieldSchema { name: "amount".to_string(), field_type: FieldType::U64 },
                FieldSchema { name: "authority".to_string(), field_type: FieldType::Pubkey },
                FieldSchema { name: "memo".to_string(), field_type: FieldType::String(32) },
            ],
        };
        let input = fuzzer.generate_random_input(&schema);
        assert!(input.fields.contains_key("amount"), "should have amount field");
        assert!(input.fields.contains_key("authority"), "should have authority field");
        assert!(input.fields.contains_key("memo"), "should have memo field");
        assert!(!input.data.is_empty(), "serialised data should be non-empty");
    }

    #[test]
    fn test_crash_oracle_classifies_overflow() {
        let config = FuzzerConfig::default();
        let fuzzer = SecurityFuzzer::new(config);
        let result = FuzzResult {
            ...
            success: false,
            error: Some("arithmetic overflow at line 42".to_string()),
            error_code: None,
            coverage_bitmap: vec![],
            interesting: false,
            is_crash: true,
        ...
        let finding_type = fuzzer.classify_crash(&result);
        assert!(
            matches!(finding_type, FindingType::ArithmeticOverflow),
            "should classify as ArithmeticOverflow"
        );
    }

    #[test]
    fn test_fuzz_loop_records_crashes() {
        let config = FuzzerConfig {
            ...
            mutations_per_input: 3,
        };
        let mut fuzzer = SecurityFuzzer::new(config);

        // Seed the corpus
        let mut fields = HashMap::new();
        fields.insert("val".to_string(), FuzzValue::U64(42));
        fuzzer.add_seed(FuzzInput {
        ...
        // Execute function: crash if val > 10_000_000
        let stats = fuzzer.fuzz(|input| {
            let val = match input.fields.get("val") {
                Some(FuzzValue::U64(v)) => *v,
                _ => 0,
            };
            let is_crash = val > 10_000_000;
            let mut bitmap = vec![0u8; 256];
            bitmap[(val % 256) as usize] = 1;
            FuzzResult {
                input: input.clone(),
                success: !is_crash,
                error: if is_crash { Some("panic: value too large".to_string()) } else { None },
                error_code: None,
        ...
        assert!(stats.total_executions > 0, "should have run some iterations");
    }

    #[test]
    fn test_coverage_tracker_unique_paths() {
        let mut tracker = CoverageTracker::new(64);
        let b1 = vec![1; 64];
        let b2 = vec![2; 64];
        tracker.is_new_coverage(&b1);
        tracker.is_new_coverage(&b2);
        assert!(tracker.unique_paths() >= 2, "two distinct bitmaps = 2 unique paths");
        // Same bitmap again should not increase count
        tracker.is_new_coverage(&b1);
        assert_eq!(tracker.unique_paths(), 2);
    }

    #[test]
    fn test_mutation_produces_diversity() {
        let config = FuzzerConfig {
            mutation_probability: 1.0, // always mutate
            mutations_per_input: 10,
            ..FuzzerConfig::default()
        };
        let mut fuzzer = SecurityFuzzer::new(config);
        let mut fields = HashMap::new();
        fields.insert("x".to_string(), FuzzValue::U64(100));
        let input = FuzzInput {
        ...
        for _ in 0..20 {
            let mutated = fuzzer.mutate_input(&input);
            if let Some(FuzzValue::U64(v)) = mutated.fields.get("x") {
                seen.insert(*v);
            }
        }
        assert!(seen.len() > 1, "mutations should produce diverse values");
    }
}
