...
//!
//! Supports `.proktor.toml` configuration files for CI-friendly operation.
//!
//! ## Example `.proktor.toml`
//!
//! ```toml
//! [scan]
...
impl Default for ProktorConfig {
    fn default() -> Self {
        Self {
...
impl ProktorConfig {
    /// Load configuration from `.proktor.toml` in the given directory.
    /// Falls back to defaults if the file doesn't exist.
    pub fn load(project_dir: &Path) -> Self {
        let config_path = project_dir.join(".proktor.toml");
        if config_path.exists() {
            match std::fs::read_to_string(&config_path) {
                Ok(content) => {
                    match toml::from_str::<ProktorConfig>(&content) {
                        Ok(config) => return config,
                        Err(e) => {
                            eprintln!("Warning: Failed to parse .proktor.toml: {}", e);
                        }
                    }
                }
                Err(e) => {
                    eprintln!("Warning: Failed to read .proktor.toml: {}", e);
                }
            }
        }
        Self::default()
...
mod tests {
    ...
    fn test_parse_toml() {
        let toml_str = r#"
            ...
            paths = ["tests/"]
        "#;
        let cfg: ProktorConfig = toml::from_str(toml_str).unwrap();
    ...
    fn test_ignore_patterns() {
        let mut cfg = ProktorConfig::default();
    ...
    fn test_load_missing_file() {
        let cfg = ProktorConfig::load(Path::new("/nonexistent"));
