...
pub enum InvariantType {
    ...
    ConstantProduct,
    /// AMM weighted pool invariant (x^w * y^(1-w) = k)
    WeightedPool,
    /// StableSwap invariant (A·sum + D = A·D + D^(n+1)/(n^n·prod))
    StablePool,
...
impl<'ctx> EconomicVerifier<'ctx> {
    ...
    pub fn verify_all_invariants(&mut self, state: &ProtocolState) -> Vec<VerificationResult> {
        vec![
            ...
            self.verify_weighted_pool(50), // 50/50 by default
            self.verify_stable_pool(85),   // A=85 (Curve-style)
            self.verify_fee_bounds(500),
            self.verify_collateralization(150),
        ]
    }

    /// Verify weighted pool invariant: x^w * y^(1-w) = k
    ///
    /// For a 50/50 pool, this reduces to standard constant product.
    /// For other weights, we use the Z3-friendly quadratic approximation:
    ///   (x_after / x_before)^weight_x * (y_after / y_before)^weight_y >= 1
    /// Rearranged to avoid division:
    ///   x_after^weight_x * y_after^weight_y >= x_before^weight_x * y_before^weight_y
    ///
    /// `weight_x_pct` is the weight of token X in percent (e.g., 80 for 80/20 pool).
    pub fn verify_weighted_pool(&mut self, weight_x_pct: u64) -> VerificationResult {
        self.solver.reset();

        let reserve_x_before = Int::new_const(self.context, "wx_before");
        let reserve_y_before = Int::new_const(self.context, "wy_before");
        let reserve_x_after  = Int::new_const(self.context, "wx_after");
        let reserve_y_after  = Int::new_const(self.context, "wy_after");
        let amount_in  = Int::new_const(self.context, "w_amount_in");
        let amount_out = Int::new_const(self.context, "w_amount_out");

        let zero = Int::from_i64(self.context, 0);
        let one  = Int::from_i64(self.context, 1);
        let weight_x = Int::from_u64(self.context, weight_x_pct);
        let weight_y = Int::from_u64(self.context, 100 - weight_x_pct);
        ...
        // State transition: sell X for Y
        let new_x = Int::add(self.context, &[&reserve_x_before, &amount_in]);
        let new_y = Int::sub(self.context, &[&reserve_y_before, &amount_out]);
        self.solver.assert(&reserve_x_after._eq(&new_x));
        self.solver.assert(&reserve_y_after._eq(&new_y));
        self.solver.assert(&new_y.ge(&zero));

        // Weighted invariant (Z3-safe integer version):
        //   x_after^w_x * y_after^w_y >= x_before^w_x * y_before^w_y
        // For Z3, we use the linearized approximation:
        //   w_x * (x_after - x_before) * y_before + w_y * (y_after - y_before) * x_before >= 0
        // This is the first-order Taylor expansion of the weighted geometric mean.
        let dx = Int::sub(self.context, &[&reserve_x_after, &reserve_x_before]);
        let dy = Int::sub(self.context, &[&reserve_y_after, &reserve_y_before]);

        let term_x = Int::mul(self.context, &[&weight_x, &dx, &reserve_y_before]);
        let term_y = Int::mul(self.context, &[&weight_y, &dy, &reserve_x_before]);
        let invariant_change = Int::add(self.context, &[&term_x, &term_y]);

        // Try to violate: invariant decreases
        let violation = invariant_change.lt(&zero);
        self.solver.assert(&violation);

        self.check_invariant(InvariantType::WeightedPool)
    }

    /// Verify stable pool (StableSwap) invariant.
    ///
    /// The Curve StableSwap formula maintains a balance between constant-sum
    /// and constant-product behavior, controlled by amplification coefficient A:
    ///   A·n·sum(x_i) + D = A·n·D + D^(n+1) / (n^n · prod(x_i))
    ///
    /// For a 2-token pool, this means the reserves should stay close to the
    /// 1:1 ratio. We verify that a swap cannot move reserves outside the
    /// allowed range determined by A.
    ///
    /// `amplification` is the A parameter (e.g., 85 for Curve's 3pool).
    pub fn verify_stable_pool(&mut self, amplification: u64) -> VerificationResult {
        self.solver.reset();

        let x_before = Int::new_const(self.context, "sx_before");
        let y_before = Int::new_const(self.context, "sy_before");
        let x_after  = Int::new_const(self.context, "sx_after");
        let y_after  = Int::new_const(self.context, "sy_after");
        let amount_in  = Int::new_const(self.context, "s_amount_in");
        let amount_out = Int::new_const(self.context, "s_amount_out");

        let zero = Int::from_i64(self.context, 0);
        let one  = Int::from_i64(self.context, 1);
        let two  = Int::from_i64(self.context, 2);
        let amp  = Int::from_u64(self.context, amplification);

        // Positivity
        self.solver.assert(&x_before.ge(&one));
        self.solver.assert(&y_before.ge(&one));
        self.solver.assert(&amount_in.ge(&one));
        self.solver.assert(&amount_out.ge(&one));

        // State transition: sell X for Y
        let new_x = Int::add(self.context, &[&x_before, &amount_in]);
        let new_y = Int::sub(self.context, &[&y_before, &amount_out]);
        self.solver.assert(&x_after._eq(&new_x));
        self.solver.assert(&y_after._eq(&new_y));
        self.solver.assert(&new_y.ge(&zero));

        // StableSwap invariant (simplified for n=2):
        //   A * 2 * (x + y) + x*y/(2^2) * 4 = A * 2 * D + D^3/(4 * x * y)  (not great for Z3)
        //
        // Instead, we check the conservation property:
        //   D_before = the virtual liquidity invariant computed from (x_before, y_before)
        //   D_after  should equal D_before (or increase due to fees)
        //
        // We use the core constraint: for a valid swap, the sum x+y should
        // not decrease faster than the product x*y / D, enforced by A.
        //
        // Simplified check: A·(sum_after) + prod_after >= A·(sum_before) + prod_before
        // where sum = x + y, prod = x * y

        let sum_before = Int::add(self.context, &[&x_before, &y_before]);
        let sum_after  = Int::add(self.context, &[&x_after, &y_after]);
        let prod_before = Int::mul(self.context, &[&x_before, &y_before]);
        let prod_after  = Int::mul(self.context, &[&x_after, &y_after]);

        // A-weighted sum + product should not decrease
        let weighted_sum_before = Int::mul(self.context, &[&amp, &two, &sum_before]);
        let weighted_sum_after  = Int::mul(self.context, &[&amp, &two, &sum_after]);

        let invariant_before = Int::add(self.context, &[&weighted_sum_before, &prod_before]);
        let invariant_after  = Int::add(self.context, &[&weighted_sum_after, &prod_after]);

        // Try to violate: invariant decreases
        let violation = invariant_after.lt(&invariant_before);
        self.solver.assert(&violation);

        self.check_invariant(InvariantType::StablePool)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_weighted_pool_50_50() {
        let ctx = create_context();
        let mut verifier = EconomicVerifier::new(&ctx);
        let result = verifier.verify_weighted_pool(50);
        println!("Weighted pool (50/50): {:?}", result);
        // For a 50/50 pool the linearized invariant should find violations
        // (since it's an approximation, the solver may find edge cases)
    }

    #[test]
    fn test_weighted_pool_80_20() {
        let ctx = create_context();
        let mut verifier = EconomicVerifier::new(&ctx);
        let result = verifier.verify_weighted_pool(80);
        println!("Weighted pool (80/20): {:?}", result);
    }

    #[test]
    fn test_stable_pool_high_amplification() {
        let ctx = create_context();
        let mut verifier = EconomicVerifier::new(&ctx);
        let result = verifier.verify_stable_pool(85);
        println!("Stable pool (A=85): {:?}", result);
    }

    #[test]
    fn test_stable_pool_low_amplification() {
        let ctx = create_context();
        let mut verifier = EconomicVerifier::new(&ctx);
        let result = verifier.verify_stable_pool(1);
        println!("Stable pool (A=1): {:?}", result);
        // Low amplification approaches constant product, so violations are more likely
    }

    #[test]
    fn test_verify_all_invariants_includes_amm_templates() {
        let ctx = create_context();
        let mut verifier = EconomicVerifier::new(&ctx);
        let state = ProtocolState::default();
        let results = verifier.verify_all_invariants(&state);
        // Should now include 9 invariant checks (7 original + weighted + stable)
        assert_eq!(results.len(), 9, "verify_all_invariants should check 9 invariants");
    }
}
