...
"#,
    ...
    fn constraint_to_kani_assertion(
        ...
        // Boolean field checks: "field" implies "account.field == true"
        if _fields.iter().any(|(name, _)| name == trimmed) {
            return format!("account.{}", trimmed);
        }

        // Default: prefix all known field names with `account.` in the expression
        let mut expr = trimmed.to_string();
        for (field_name, _) in _fields {
            expr = expr.replace(field_name.as_str(), &format!("account.{}", field_name));
        }
        format!("{{ {} }}", expr)
    }
}
