...
mod tests {
    ...
    #[test]
    fn test_arithmetic_visitor_detects_unchecked_ops() {
        let source = r#"
            pub fn transfer_tokens(amount: u64, fee: u64) -> u64 {
        ...
        assert_eq!(visitor.functions_analyzed, 1);
        assert!(!visitor.unchecked_ops.is_empty(), "Should detect unchecked arithmetic");
        assert_eq!(visitor.unchecked_ops[0].0, "transfer_tokens");
    }

    #[test]
    fn test_arithmetic_visitor_detects_checked_ops() {
        let source = r#"
        ...
        let mut visitor = ArithmeticVisitor::new();
        syn::visit::Visit::visit_file(&mut visitor, &file);
        assert_eq!(visitor.functions_analyzed, 2);
        assert!(visitor.checked_ops.len() >= 2, "Should detect checked_add and saturating_sub");
    }

    #[test]
    fn test_z3_multiplication_overflow_128bit() {
        use z3::ast::BV;
        use z3::{Config, Context, SatResult, Solver};
        ...
        // a and b are modest values but their product overflows u64
        let bound = BV::from_u64(&ctx, 1u64 << 32, 64);
        solver.assert(&a.bvuge(&bound)); // a >= 2^32
        solver.assert(&b.bvuge(&bound)); // b >= 2^32

        // Widen to 128-bit and check if product > u64::MAX
        let a_wide = a.zero_ext(64);
        let b_wide = b.zero_ext(64);
        let product = a_wide.bvmul(&b_wide);
        let max_64 = BV::from_u64(&ctx, u64::MAX, 128);
        ...
        // 2^32 * 2^32 = 2^64 > u64::MAX, so SAT
        assert_eq!(solver.check(), SatResult::Sat);
    }

    #[test]
    fn test_report_serialization_roundtrip() {
        let report = Layer2Report {
            status: "Passed".to_string(),
            findings_count: 0,
            analyzed_instructions: 42,
            confidence: 0.95,
            duration_ms: 100,
            z3_proofs: vec![Z3ProofResult {
                property: "supply_invariant".to_string(),
                status: ProofStatus::Proven,
                description: "Supply is bounded".to_string(),
                counterexample: None,
        ...
        let deserialized: Layer2Report = serde_json::from_str(&json).unwrap();
        assert_eq!(deserialized.status, "Passed");
        assert_eq!(deserialized.z3_proofs.len(), 1);
        assert_eq!(deserialized.z3_proofs[0].status, ProofStatus::Proven);
    }

    #[tokio::test]
    async fn test_verify_on_vulnerable_token() {
        let verifier = Layer2Verifier::new();
        let target = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent().unwrap()
            .parent().unwrap()
            .parent().unwrap()
            .join("programs/vulnerable-token/src");
        if target.exists() {
            let report = verifier.verify(&target).await.unwrap();
            assert!(report.total_functions_analyzed > 0, "Should analyze functions");
