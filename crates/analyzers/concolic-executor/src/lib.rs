...
mod tests {
    ...
    #[test]
    fn test_z3_solves_simple_constraint() {
        let config = ConcolicConfig::default();
        let executor = ConcolicExecutor::new(config);
        // Z3 should find `amount > 100`
        let conditions = vec![PathCondition {
            constraint: "amount > 100".to_string(),
            location: "transfer.rs:5".to_string(),
            branch_taken: true,
        }];
        let result = executor.solve_constraints(&conditions);
        assert!(result.is_some(), "Z3 should find a satisfying assignment");
        let vals = result.unwrap();
        assert!(
            vals.get("amount").copied().unwrap_or(0) > 100,
            "solved value should satisfy amount > 100"
        );
    }

    #[test]
    fn test_execute_with_boundary_inputs_explores_paths() {
        ...
        // Provide boundary values to exercise overflow paths
        let mut inputs = HashMap::new();
        inputs.insert("amount".to_string(), u64::MAX);
        inputs.insert("balance".to_string(), 0);
        let result = executor.execute(inputs);
        assert!(result.paths_explored > 0, "should explore at least one path");
        assert!(
            !result.test_inputs.is_empty(),
            "should generate at least one test input"
        );
    }

    #[test]
    fn test_branch_negation_spawns_alternative_paths() {
        let config = ConcolicConfig {
            max_depth: 3,
            max_paths: 10,
            timeout_ms: 1000,
        ...
        let mut executor = ConcolicExecutor::new(config);
        // Manually add a branch condition to the initial state
        let mut state = ExecutionState::new();
        state.concrete_values.insert("x".to_string(), 50);
        executor.add_branch_condition(
            &mut state,
            "x > 10".to_string(),
            true,
            "check.rs:3".to_string(),
        );
        // Enqueue and execute
        executor.worklist.push_back(state);
        let result = executor.execute(HashMap::new());
        // The executor should have explored at least the original path
        assert!(
            result.paths_explored >= 1,
            "should explore original + potentially negated path"
        );
    }

    #[test]
    fn test_z3_multi_variable_constraints() {
        let config = ConcolicConfig::default();
        let executor = ConcolicExecutor::new(config);
        let conditions = vec![
            PathCondition {
                constraint: "x > 0".to_string(),
                location: "lib.rs:1".to_string(),
                branch_taken: true,
            },
            PathCondition {
                constraint: "y < 1000".to_string(),
                location: "lib.rs:2".to_string(),
                branch_taken: true,
            },
        ];
        let result = executor.solve_constraints(&conditions);
        assert!(result.is_some(), "Z3 should solve multi-variable system");
        let vals = result.unwrap();
        assert!(vals.get("x").copied().unwrap_or(0) > 0);
        assert!(vals.get("y").copied().unwrap_or(u64::MAX) < 1000);
    }
}
