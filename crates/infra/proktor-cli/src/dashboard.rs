...
const PHASES: [(Color, &str, &str); 14] = [
    // Batch 1: Core (1-11) - logical grouping
    (C_RED, "PATTERN", "72 heuristic rules"),
    (C_PURPLE, "DEEP-AST", "syn::Visit traversal"),
    (C_YELLOW, "TAINT", "Lattice info-flow"),
    (Color::Rgb(0, 220, 255), "CFG", "Dominator proofs"),
    ...
    // Batch 2: Heuristics (12-16)
    (C_RED, "GEIGER", "Unsafe code analyzer"),
    (C_ORANGE, "L3X/HEUR", "Pattern reasoning"),
    // Batch 3: Formal Verification (17-23)
    (C_ACCENT, "FV/Z3", "SMT provers"),
    (C_PURPLE, "CONCOLIC", "Path exploration"),
    // Batch 4: Mathematical Proofs (24-28)
    (C_GREEN, "MATH-PROV", "Octagon/SepLogic/CTL"),
];
...
fn render_header(f: &mut Frame, area: Rect, state: &DashboardState) {
    ...
    let title = Line::from(vec![
        Span::styled(" PROKTOR ", Style::default().fg(C_BG).bg(C_ACCENT).add_modifier(Modifier::BOLD)),
        ...
        Span::styled("28 Phases ", Style::default().fg(C_MUTED)),
        Span::styled("| ", Style::default().fg(C_BORDER)),
...
fn render_help(f: &mut Frame, area: Rect) {
    let lines = vec![
        ...
        Line::from(Span::styled(
            "  PROKTOR SECURITY CONSOLE -- KEYBOARD SHORTCUTS",
            Style::default().fg(C_ACCENT).add_modifier(Modifier::BOLD),
        )),
        Line::from(""),
...
fn compute_phase_counts(findings: &[program_analyzer::VulnerabilityFinding]) -> [usize; 14] {
    ...
    for f in findings {
        ...
        let idx = if id.starts_with("SOL-DEEP") {
            ...
            7 // Anchor
        } else if id.starts_with("SOL-090") || id.starts_with("SOL-091") {
            8 // Dataflow
        } else if id == "SOL-093" {
            9 // Geiger
        } else if id.starts_with("L3X") {
            10 // L3X
        } else if id.starts_with("SOL-FV") || id.starts_with("SOL-SYM") {
            11 // Formal verification
        } else if id == "SOL-096" {
            12 // Concolic
        } else if id.starts_with("SOL-OCT") || id.starts_with("SOL-SEP") || id.starts_with("SOL-IFC") || id.starts_with("SOL-CTL") || id.starts_with("SOL-COMP") {
