...
struct ProktorLspBackend {
    client: Client,
}

#[tower_lsp::async_trait]
impl LanguageServer for ProktorLspBackend {
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.client
            .log_message(MessageType::INFO, "Proktor LSP server initialized — 72+ vulnerability patterns active")
            .await;
    }
...
async fn main() {
    ...
    let (service, socket) = LspService::new(|client| ProktorLspBackend { client });
