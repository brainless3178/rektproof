//! SVG Badge Generator for Proktor Security Scores
//!
...
pub fn generate_badge_svg(score: u8, name: &str) -> String {
    ...
    svg.push_str(&format!(
        r#"<svg xmlns="http://www.w3.org/2000/svg" width="{total_w}" height="20" role="img" aria-label="Proktor: {score}/100">"#
    ));
    svg.push_str(&format!(
        r#"<title>Proktor: {safe_name} {score}/100 ({grade})</title>"#
    ));
    svg.push_str(&format!(
    ...
    svg.push_str(&format!(
        r#"<text x="{label_cx}" y="15" fill="{shadow}" fill-opacity=".3">Proktor Verified</text>"#
    ));
    svg.push_str(&format!(
        r#"<text x="{label_cx}" y="14" fill="{white}">Proktor Verified</text>"#
    ));
    svg.push_str(&format!(
