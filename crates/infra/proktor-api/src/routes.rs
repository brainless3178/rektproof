...
pub async fn health(state: web::Data<AppState>) -> HttpResponse {
    ...
    HttpResponse::Ok().json(serde_json::json!({
        "status": "ok",
        "service": "proktor-security-oracle",
        "version": "0.1.0",
...
pub async fn list_engines(_state: web::Data<AppState>) -> HttpResponse {
    ...
    if let Ok(entries) = std::fs::read_dir("./crates") {
        ...
        for name in &names {
            let (engine_type, is_analysis) = match name.as_str() {
                "proktor-api" => ("REST Gateway", false),
                "proktor-extension" => ("Client SDK", false),
                "proktor-cli" => ("CLI Interface", false),
                ...
                "proktor-oracle" | "consensus-engine" => ("On-Chain Oracle", true),
                ...
                "proktor-guard" => ("Supply Chain Firewall", true),
                "proktor-monitor" => ("Authority Monitor", true),
                "proktor-verify" => ("Verification Engine", true),
    ...
    if engines.is_empty() {
        return HttpResponse::Ok().json(serde_json::json!([
            { "name": "proktor-api", "type": "REST Gateway", "status": "active", "detectors": 0, "is_analysis_engine": false }
        ]));
    }
...
// ─── Proktor Guard — Dependency Firewall ─────────────────────────────────────
...
/// POST /api/v1/guard
/// Run the Proktor Guard dependency firewall on a directory or GitHub repo.
...
pub async fn scoreboard_scan(
    ...
    let (findings_summary, guard_risk) = if let Some(repo_url) = &body.repo_url {
        ...
        let temp_dir = std::env::temp_dir().join(format!("proktor-score-{}", &program_id[..8.min(program_id.len())]));
        ...
        let scan_result: Result<(crate::scoreboard::FindingSummary, u8), String> = tokio::task::spawn_blocking({
            ...
            move || {
                ...
                // Run guard scan
                let guard_scanner = proktor_guard::GuardScanner::new();
