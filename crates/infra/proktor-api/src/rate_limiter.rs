//! # Rate Limiter Middleware for Proktor API
//!
...
//! |----------|---------|-------------|
//! | `PROKTOR_RATE_LIMIT_RPS` | `30` | Requests per second per IP |
//! | `PROKTOR_RATE_LIMIT_BURST` | `60` | Maximum burst capacity |
//!
//! ## Usage
//!
//! ```rust,ignore
...
impl RateLimiterState {
    ...
    ///
    /// - `PROKTOR_RATE_LIMIT_RPS`: requests per second (default: 30)
    /// - `PROKTOR_RATE_LIMIT_BURST`: burst capacity (default: 60)
    pub fn from_env() -> Self {
        let rps: f64 = std::env::var("PROKTOR_RATE_LIMIT_RPS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(30.0);
        let burst: u32 = std::env::var("PROKTOR_RATE_LIMIT_BURST")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(60);
        Self::new(rps, burst)
