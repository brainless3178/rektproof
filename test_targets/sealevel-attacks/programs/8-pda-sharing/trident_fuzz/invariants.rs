...
/// Invariant: PDA addresses must be derived from canonical bumps.
pub fn check_pda_canonical_bump(
    pda_address: &Pubkey,
    seeds: &[&[u8]],
    program_id: &Pubkey,
) -> InvariantResult {
    let (expected, _bump) = Pubkey::find_program_address(seeds, program_id);
    if pda_address != &expected {
        InvariantResult::Violated(format!(
            "PDA address mismatch: got {}, expected {}",
            pda_address, expected,
        ))
    } else {
        InvariantResult::Held
    }
